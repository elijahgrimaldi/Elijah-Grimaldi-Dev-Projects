...
def viewStr(instances): return ','.join(r.socket_address for r in instances)
def viewSet(instances): return set(r.socket_address for r in instances)
...
def runInstance(instance, view_instances, shard_count=None):
    ...
    command = ['docker', 'run', '--rm', '--detach',
               '--publish={}:{}'.format(instance.published_port,
                                        containerPort),
...
class TestHW4(unittest.TestCase):
    ...
    # class/static properties used as shared-state (via internal mutability) across the whole test suite
    shard_ids = list()  # List of shard ids
    shard_members = dict()  # Dictionary from shard ids to member lists
    # Dictionary from key "metadata" to metadata returned by an instance
    causal_metadata = dict(metadata=0)
    ...
    def setUpClass(cls):
        ...
        # grace is the extra instance not part of all_instances
        stopAndRemoveInstance(grace, required=False)
    ...
    def tearDownClass(cls):
        ...
        # grace is the extra instance not part of all_instances
        killInstance(grace, required=False)
        for instance in all_instances:
    ...
    def test_a_get_shard_ids(self):
        ...
        response = requests.get(
            'http://{}:{}/shard/ids'.format(hostname, alice.published_port))
        ...
        for instance in all_instances:
            with self.subTest(msg='at instance {}'.format(instance)):
                response = requests.get(
                    'http://{}:{}/shard/ids'.format(hostname, instance.published_port))
                self.assertEqual(response.status_code, 200)
                self.assertIn('shard-ids', response.json())
                self.assertEqual(
                    set(response.json()['shard-ids']), set(shard_ids))
    ...
    def test_b_shard_id_members(self):
        ...
        for shard_id in self.shard_ids:
            with self.subTest(msg='for shard {}'.format(shard_id)):
                ...
                response = requests.get(
                    'http://{}:{}/shard/members/{}'.format(hostname, alice.published_port, shard_id))
                self.assertEqual(response.status_code, 200)
    ...
    def test_c_node_shard_id(self):
        ...
        for instance in all_instances:
            with self.subTest(msg='at instance {}'.format(instance)):
                ...
                response = requests.get(
                    'http://{}:{}/shard/node-shard-id'.format(hostname, instance.published_port))
                self.assertEqual(response.status_code, 200)
                self.assertIn('node-shard-id', response.json())
                instance_shard_id = response.json()['node-shard-id']

                print('=== Check that the {}\'s id {} fits with previously collected data'.format(
                    instance, instance_shard_id))
                self.assertIn(instance_shard_id, self.shard_ids)
                self.assertIn(instance_shard_id, self.shard_members)
                self.assertIn(instance.socket_address,
                              self.shard_members[instance_shard_id])
    ...
    def test_d_put_key_value_operation(self):
        ...
        for n in range(self.key_count):
            ...
            print('>>> Put {key}:{value} at instance {instance} (with retries)'.format(
                key=key, value=value, instance=instance))

            retries = 7
            def backoffSec(attempt): return 0.01*2**attempt
            ...
            for attempt in range(retries):
                ...
                print('Try {attempt}/{retries} PUT {key}:{value} -> {instance} -> {code} @{m}'.format(key=key, value=value,
                      instance=instance, m=self.causal_metadata['metadata'], attempt=attempt + 1, retries=retries, code=response.status_code))
                if response.status_code == 503:
                    sleep(backoffSec(attempt))
                    continue  # retry
                else:
                    self.assertEqual(response.status_code, 201)
                    self.causal_metadata['metadata'] = response.json()[
                        'causal-metadata']
                    break  # next request
            else:
    ...
    def test_e_get_key_value_operation(self):
        ...
        print('=== Check for correctness of {} key:value pairs in the store.'.format(
            self.key_count))
        for n in range(self.key_count):

            key = 'key{}'.format(n)
            value = 'value{}'.format(n)
            # different instance from the one chosen above
            instance = all_instances[(n + 1) % len(all_instances)]
            print('=== Check {key}:{value} at instance {instance}'.format(
                key=key, value=value, instance=instance))
            ...
            print('GET {key} -> {instance} -> {code} @{m}'.format(key=key, instance=instance,
                  m=self.causal_metadata['metadata'], code=response.status_code))
            self.assertEqual(response.status_code, 200)
    ...
    def test_f_shard_key_count(self):
        ...
        for shard_id, members in self.shard_members.items():
            ...
            with self.subTest(msg='for shard {}'.format(shard_id)):
                print('... The first instance in shard {} is {}'.format(
                    shard_id, members[0]), end=', ')
                (first_instance,) = [
                    instance for instance in all_instances if instance.socket_address == members[0]]
                print(first_instance)

                print(
                    '>>> Get key-count for shard {} from {}'.format(shard_id, first_instance))
                response = requests.get(
                    'http://{}:{}/shard/key-count/{}'.format(hostname, first_instance.published_port, shard_id))
                self.assertEqual(response.status_code, 200)
            ...
            print('=== Check that everybody reports key-count {} for shard {}'.format(
                shard_key_counts[shard_id], shard_id))
            for instance in all_instances:
                with self.subTest(msg='for shard {}; at instance {}'.format(shard_id, instance)):
                    response = requests.get(
                        'http://{}:{}/shard/key-count/{}'.format(hostname, instance.published_port, shard_id))
                    self.assertEqual(response.status_code, 200)
                    self.assertIn('shard-key-count', response.json())
                    self.assertEqual(shard_key_counts[shard_id], response.json()[
                                     'shard-key-count'])
    ...
    def test_g_add_new_node(self):
        ...
        for instance in all_instances:
            with self.subTest(msg='at instance {}'.format(instance)):
                response = requests.get(
                    'http://{}:{}/view'.format(hostname, instance.published_port))
                self.assertEqual(response.status_code, 200)
        ...
        print('>>> Assign {} to shard {} with a request to {}'.format(
            grace, assigned_shard, shard_assigner))
        ...
        response = requests.get(
            'http://{}:{}/shard/node-shard-id'.format(hostname, grace.published_port))
        self.assertEqual(response.status_code, 200)
        self.assertIn('node-shard-id', response.json())
        self.assertEqual(response.json()['node-shard-id'], assigned_shard)

        print('=== Check that {} is in shard {} members at each instance'.format(
            grace, assigned_shard))
        for instance in all_instances:
            with self.subTest(msg='at instance {}'.format(instance)):
                response = requests.get(
                    'http://{}:{}/shard/members/{}'.format(hostname, instance.published_port, assigned_shard))
                self.assertEqual(response.status_code, 200)
                self.assertIn('shard-members', response.json())
                self.assertIn(grace.socket_address,
                              response.json()['shard-members'])

        print('=== Check that all instances agree about shard {} key-count'.format(assigned_shard))

        response = requests.get(
            'http://{}:{}/shard/key-count/{}'.format(hostname, grace.published_port, assigned_shard))
        self.assertEqual(response.status_code, 200)
        self.assertIn('shard-key-count', response.json())
        reported_key_count = response.json()['shard-key-count']

        print('=== Check that everybody reports key-count {} for shard {}'.format(
            reported_key_count, assigned_shard))
        for instance in all_instances:
            with self.subTest(msg='at instance {}'.format(instance)):
                response = requests.get(
                    'http://{}:{}/shard/key-count/{}'.format(hostname, instance.published_port, assigned_shard))
                self.assertEqual(response.status_code, 200)
                self.assertIn('shard-key-count', response.json())
                self.assertEqual(
                    response.json()['shard-key-count'], reported_key_count)

    def test_h_impossible_reshard(self):

        print('=== Check that an impossible reshard is rejected')
        instance = random.choice(all_instances)
        response = requests.put('http://{}:{}/shard/reshard'.format(
            hostname, instance.published_port), json={'shard-count': 10})
    ...
    def test_i_possible_reshard(self):
        ...
        with self.subTest(msg='reshard at {}'.format(instance)):
            response = requests.put('http://{}:{}/shard/reshard'.format(
                hostname, instance.published_port), json={'shard-count': 3})
            self.assertEqual(response.status_code, 200)
        ...
        for instance in all_instances:
            with self.subTest(msg='at instance {}'.format(instance)):
                ...
                print('=== Check that the {}\'s shard id {} fits with previously collected data'.format(
                    instance, instance_shard_id))
                self.assertIn(instance_shard_id, shard_members)
                self.assertIn(instance.socket_address,
                              shard_members[instance_shard_id])
        ...
        for n in range(self.key_count):
            ...
            value = 'value{}'.format(n)
            # different instance from the one chosen above
            instance = all_instances[(n + 2) % len(all_instances)]
            print('=== Check {key}:{value} at instance {instance}'.format(
                key=key, value=value, instance=instance))
